...
import { DataPageDropdown } from "../_components";
...
class PegaForm extends Component {
          ...
          {layout.rows.map((row, childIndex) => {
            return (
              <Table.Row key={childIndex}>
                {layout.referenceType === gridTypes.GROUP && (
                  <Table.Cell>
                    {ReferenceHelper.getPageGroupKeyFromRow(row)}
                  </Table.Cell>
                )}
  ...
  createCaption(caption, index) {
    return <div key={index}>{caption.value}</div>;
  }
  ...
  createField(field, index, showLabel) {
    ...
    const handleChange = (e, data) => this.handleChange(e, data);
      ...
      case fieldTypes.ICON:
        ...
        switch (iconMode.iconSource) {
          case iconSources.STANDARD:
            icon = <Icon name={standardIcons[iconMode.iconStandard]} />;
            break;
          ...
          case iconSources.EXTERNAL_URL:
            icon = <img src={iconMode.iconUrl} alt="Icon from external URL" />;
            break;
          ...
          case iconSources.STYLECLASS:
            icon = (
              <i className={iconMode.iconStyle} alt="Icon from styleclass" />
            );
            break;
          default:
            icon = <img alt="Icon with undefined source" />;
            break;
        }
  ...
  getPropertyValue(property) {
    ...
    // If the property is a number, parse it and return it
    if (!isNaN(property)) {
      return parseInt(property, 10);
    }

    // If the property is a bool in a string, then return a bool
  ...
  handleGroupActions(e, data) {
    ...
    const name = prompt("Please enter a name for the group.", "");
    if (name === null) {
      return;
    }
