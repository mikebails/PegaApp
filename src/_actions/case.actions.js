...
function updateCase(myCase, action, body) {
  return dispatch => {
    dispatch(request(myCase.caseID));

    caseService.updateCase(myCase, action, body).then(
      aCase => {
        dispatch(success(aCase));
        dispatch(caseActions.refreshCase(myCase.ID));
      },
      error => {
        dispatch(failure(error));
        dispatch(alertActions.error(error));
